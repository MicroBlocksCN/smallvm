...
static int stringSize(OBJ obj) {
	// Note: This temporary code will smooth the transition to new header format that
	// supports ByteArrays. Although less efficient, using strlen() works with string
	// constants generated by both old and the new compilers.

	if (StringType != objType(obj)) return 0;
	return strlen(obj2str(obj));

	// The following code can be reinstated a few releases after 0.4.1:
